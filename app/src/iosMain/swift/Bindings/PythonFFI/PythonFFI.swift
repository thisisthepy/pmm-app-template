...
@objc(pyInitialize) public func pyInitialize() -> Void {
    ...
    // Keep this in case of using kivy
...
@objc(pyFinalize) public func pyFinalize() -> Void {
    Py_Finalize()
}

@objc(pyImport) public func pyImport(module: NSString) -> UInt64 {
    let pointer: UnsafePointer<CChar>? = module.utf8String
    return PyImport_Import(PyUnicode_DecodeFSDefault(pointer))
}

@objc(pyErrorOccurred) public func pyErrorOccurred() -> Bool {
    return PyErr_Occurred()
}

@objc(pyErrorPrint) public func pyErrorPrint() -> Void {
    PyErr_Print()
}

@objc(pyObjectGetAttrString) public func pyObjectGetAttrString(obj: UInt64, attr: NSString) -> UInt64 {
...
@objc(pyObjectCallObject) public func pyObjectCallObject(callable: UInt64, args: UInt64) -> UInt64 {
